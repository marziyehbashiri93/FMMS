...
| **Validation Status**   | M8 entry corrections PASS — 508/508 tests; black/isort/ruff/mypy PASS; awaiting final M8 approval |
...
| Field         | Value                                                                       |
|---------------|-----------------------------------------------------------------------------|
| **Status**    | `Pending` (entry corrections applied; coding not started)                   |
...
- [ ] Configure `CELERY_BEAT_SCHEDULE` in `base.py` (reduced scope only):
  ...
  - `sync-equipment-single`: `sync_equipment_from_sap` — schedule TBD (single equipment; not bulk)
...
- [ ] Create `infrastructure/messaging/tasks/sap_sync_tasks.py`:
  - [ ] `sync_equipment_from_sap` — calls `SyncSAPEquipmentService` only (single equipment)

**SAP Retry Tasks**
- [ ] Create `infrastructure/messaging/tasks/sap_retry_tasks.py`:
  - [ ] `retry_failed_sap_transactions` — calls application service / `ISAPTransactionManager` facade
        (`retry` / `retry_all_pending`); **never** queries ORM models from the task

**Maintenance Scheduling Tasks**
- [ ] Create `infrastructure/messaging/tasks/maintenance_tasks.py`:
  - [ ] `trigger_overdue_pm_work_orders` — calls `TriggerPMWorkOrderService` for overdue plans
        (SAP notification writes still go through `ISAPTransactionManager`)

**Task Quality Standards (mandatory)**
- [ ] All tasks decorated with `@shared_task(bind=True, max_retries=3)`
- [ ] Tasks may only: resolve dependencies, call application services or manager facade, log
- [ ] Tasks must never: query ORM directly, contain business logic
- [ ] All tasks use `try/except` with structured logging — zero silent failures
- [ ] Every task log includes: `domain`, `task_name`, `task_id`, `correlation_id`
- [ ] Use `get_structured_logger` (not stdlib logger alone)

**Task Tests**
- [ ] `tests/unit/infrastructure/messaging/test_sap_sync_tasks.py` — mocked services
- [ ] `tests/unit/infrastructure/messaging/test_sap_retry_tasks.py` — mocked manager/service facade
...
*Last updated: 2026-07-10 | Updated by: Lead Backend Architect | Validation: M8 entry corrections committed — SAP write gateway unified (508/508 tests); M8 coding not started*
