...
Workflow (designed for a future transaction boundary):
...
5. Optionally create a SAP PM notification via ``ISAPPMNotificationPort`` only
   (never infrastructure adapters / SAPTransactionManager).
...
class TriggerPMWorkOrderService:
    ...
    def execute(self, dto: TriggerPMWorkOrderDTO) -> PMWorkOrderResponseDTO:
        ...
        Raises:
            ...
            FMMSConflictError: If the plan is not ACTIVE, or SAP notification
                was requested without equipment / port.
    ...
    def _create_sap_notification(
        ...
        """Create a SAP PM notification via the injected port only.
        ...
        Raises:
            FMMSConflictError: If port is missing or vehicle has no equipment number.
        ...
        response = self._sap.create_notification(
        ...
        return response.notification_number
