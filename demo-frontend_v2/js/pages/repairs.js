...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const view = params.get("view") || document.body.dataset.view || "orders";
  const scope = params.get("scope") || document.body.dataset.scope || "transport";
  if (page === "transport" && view === "final-approval") {
    return;
  }
  if (page === "transport") {
    FMMS.pageBoot?.initPage("transport-repairs", () => FMMS.pages.repair?.renderTransport?.("repairs"));
  } else if (page === "materials") {
    const pageId = scope === "workshop" ? "workshop-materials" : "transport-materials";
    FMMS.pageBoot?.initPage(pageId, () => {
      if (scope === "workshop") return FMMS.pages.repair?.renderWorkshop?.("materials");
      return FMMS.pages.repair?.renderTransport?.("materials");
    });
  } else if (page === "invoices") {
    FMMS.pageBoot?.initPage("transport-invoices", () => FMMS.pages.repair?.renderTransport?.("invoices"));
  } else {
    const pageId = view === "history" ? "workshop-history" : "workshop-orders";
    FMMS.pageBoot?.initPage(pageId, () => FMMS.pages.repair?.renderWorkshop?.(view === "history" ? "history" : "orders"));
  }
});
