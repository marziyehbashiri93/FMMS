...
document.addEventListener("DOMContentLoaded", () => {
  const view = new URLSearchParams(location.search).get("view") || document.body.dataset.view || "inspection";
  if (view === "handover") {
    FMMS.pageBoot?.initPage("handover", () => FMMS.pages.handover?.render?.());
  } else {
    FMMS.pageBoot?.initPage("inspection", () => FMMS.pages.inspection?.render?.());
  }
});
